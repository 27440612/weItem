				...
				<div class="listshow" v-if="ssNum==1">
					...
					<div class="listshow2" v-if="ssNum==3">
						...
						<ul class="filter_ul filter_ulOne">
							   <li class="filter_li" v-for="(i,$index) in filterText" @click="allBlue(i,$index)" :style="{'color':blueIndex[$index+1]?'#3190E8':''}">
...
//			console.log(i)
			...
			if(!this.blueIndex[e + 1]) {
				...
				for(var r = 0; r < this.last.length; r++) {
					if(this.last.indexOf(i.id) > -1) {
						this.last.splice(this.last.indexOf(i.id), 1)
					}
				}
			}
